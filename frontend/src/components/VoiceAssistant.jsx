...
const VoiceAssistant = ({ onVoiceCommand }) => {
  ...
  const [useWhisper, setUseWhisper] = useState(false); // Use browser Speech Recognition as primary method
  ...
  useEffect(() => {
    console.log('🔧 VoiceAssistant useEffect running');
    ...
    try {
      ...
      if (!isSecureContext) {
        console.log('⚠️ Not secure context, disabling speech recognition');
        setHttpsRequired(true);
        setSpeechSupported(false);
        return;
      }

      // Initialize browser speech recognition as primary method
      console.log('🗣️ Using Browser Speech Recognition API for better accuracy');
      
      // MediaRecorder available as backup if needed
      if (navigator.mediaDevices && MediaRecorder.isTypeSupported('audio/webm')) {
        console.log('✅ MediaRecorder available as backup');
      }
      ...
      recognition.onresult = (event) => {
        ...
        if (result.isFinal) {
          ...
          for (let i = 0; i < result.length; i++) {
            if (result[i].confidence > bestAlternative.confidence) {
              bestAlternative = result[i];
            }
          }
  ...
  const startListening = () => {
    ...
    // Use browser speech recognition as primary method
    if (recognitionRef.current) {
      try {
      ...
      // Fallback to Whisper if speech recognition not available
  ...
  const stopListening = () => {
    ...
    } else if (useWhisper && mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
      mediaRecorderRef.current.stop();
    }
  ...
  const processVoiceCommand = async (command) => {
    ...
    try {
      ...
      console.log('🎙️ Voice command API response:', data);
      ...
      if (response.ok) {
        ...
        if (onVoiceCommand) {
          console.log('🎯 Calling onVoiceCommand with data:', {
            ...data,
            transcript: command,
            source: 'voice_text'
          });
          ...
          console.log('⚠️ No onVoiceCommand callback provided');
        }
  ...
  const processAudioCommand = async (audioBlob) => {
    try {
      ...
      // Handle all command actions (habits, navigation, etc.)
      if (data.action_taken && (data.habit_action || data.frontend_action)) {
        console.log('🎯 Command action taken:', data);
        if (onVoiceCommand) {
          onVoiceCommand(data);
        }
      }
...
Try: 'go to analytics' or 'add habit to meditate'"
            ...
            <div className="voice-help">
              ...
              <div className="speech-recognition-info">
                <p><strong>�️ Browser Speech Recognition:</strong></p>
                <p>Fast, accurate, local speech processing with full app control!</p>
                <p><strong>💡 Tip:</strong> Speak clearly and naturally for best results</p>
                <p><strong>🔧 Quick Test:</strong> Try saying "go to analytics" or type commands above</p>
              </div>
            </div>
          </div>
        </div>
